...
## Context
Route handlers currently contain SQLAlchemy queries directly. This couples
business logic to the ORM and makes unit testing require a real database.

## Decision
Introduce a repository layer (TodoRepository, UserRepository) that abstracts
all database access. Business logic and route handlers depend on the repository
interface (Protocol), not on SQLAlchemy directly.
